...
const TopPerformers = () => {
  ...
  useEffect(() => {
    const load = async () => {
      const { data } = await supabase.from("top_performers").select("*");
      if (data && data.length > 0) {
        const updated = { ...defaultPerformers };
  ...
  const playerPhoto = photoMap[active.key];
      ...
      <div className="container relative z-10">
        ...
        <div className="relative touch-pan-y">
          <AnimatePresence initial={false} custom={direction} mode="wait">
            <motion.div key={active.key} custom={direction} variants={variants} initial="enter" animate="center" exit="exit"
              transition={{ type: "spring", stiffness: 300, damping: 30 }} drag="x" dragConstraints={{ left: 0, right: 0 }}
              dragElastic={1} onDragEnd={handleDragEnd}
              ...
              <div className="flex flex-col md:flex-row">
                <div className="relative w-full md:w-2/5 flex items-end justify-center p-6 pt-10 md:p-8 min-h-[280px] md:min-h-[380px]">
                  ...
                  <img
                    src={playerPhoto || playerImage}
                    alt={player.name}
                    className={`relative z-10 ${playerPhoto ? "h-[220px] md:h-[320px] w-[220px] md:w-[320px] object-cover rounded-2xl" : "h-[220px] md:h-[320px] object-contain"} drop-shadow-2xl`}
                  />
                </div>
        ...
        <div className="flex justify-center gap-2 mt-3">
          {categories.map((cat, i) => (
            <button key={cat.key} onClick={() => { setDirection(i > activeIdx ? 1 : -1); setActiveIdx(i); }}
              className={`h-2.5 rounded-full transition-all ${i === activeIdx ? `${cat.bgColor} w-8` : "bg-border w-2.5"}`}
